...
class Pagination
{
    ...
    public function get()
    {
        ...
        if (!is_null($links)) {
            ...
            # Создаём ссылку "На первую"
                $links = $this->generateHtml(1, '<i class="material-icons">fast_rewind</i>') . $links;
            # Если текущая страница не первая
            if ($this->current_page < $this->amount)
            # Создаём ссылку "На последнюю"
                $links .= $this->generateHtml($this->amount, '<i class="material-icons">fast_forward</i>');
        }
        $html .= $links . '</ul>';
        # Возвращаем html
        return $html;
    ...
    private function generateHtml($page, $text = null)
    {
        ...
        return
                '<li><a class="waves-effect" href="' . $currentURI . $this->index . $page . '">' . $text . '</a></li>';
    }
    /**
